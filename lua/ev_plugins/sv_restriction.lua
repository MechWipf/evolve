...
function PLUGIN:PlayerSpawnSWEP( ply, name, tbl )
	if ( GAMEMODE.IsSandboxDerived and table.HasValue( evolve.privileges, "@" .. name ) and not ply:EV_HasPrivilege( "@" .. name ) ) then
...
function PLUGIN:PlayerSpawnSENT( ply, class )
	if ( GAMEMODE.IsSandboxDerived and table.HasValue( evolve.privileges, ":" .. class ) and not ply:EV_HasPrivilege( ":" .. class ) ) then
...
function PLUGIN:PlayerCanPickupWeapon( ply, wep )
	if ( GAMEMODE.IsSandboxDerived and table.HasValue( evolve.privileges, "@" .. wep:GetClass() ) and not ply:EV_HasPrivilege( "@" .. wep:GetClass() ) and ( not ply.EV_PickupTimeout or CurTime() < ply.EV_PickupTimeout ) ) then
		return false
	end
end
...
-- 	if ( not evolve:GetGlobalVar( "RestrictionSetUp", false ) ) then		
...
-- 	if ( not evolve:GetGlobalVar( "RestrictionSetUpEnts", false ) ) then		
-- 		for id, rank in pairs( evolve.ranks ) do
-- 			if ( id ~= "owner" ) then
...
-- 	if ( not evolve:GetGlobalVar( "RestrictionSetUpTools2", false ) ) then		
-- 		for id, rank in pairs( evolve.ranks ) do
-- 			if ( id ~= "owner" ) then
